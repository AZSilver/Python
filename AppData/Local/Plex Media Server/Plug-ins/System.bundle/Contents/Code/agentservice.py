...
class AgentService(SystemService):
  ...
  def search(self, identifier, mediaType, lang, manual=False, **kwargs):
    ...
    try:
      ...
      result = self.agent_search(identifier, media_type, lang, manual, kwargs)
      
      # Convert each item's ID into a GUID
      if MediaContainer.is_instance(result):
  ...
  def update(self, mediaType, guid, id=None, parentGUID=None, force=False, agent=None, async=True):
    """
  ...
  def get_relpath(self, media_type, identifier, guid):
    ...
    primary_metadata_cls = getattr(primary_access_point, media_type)
    try:
  ...
  def update_task(self, mediaType, media_type, identifier, guid, id, lang, dbid, parentGUID, force, agent, async):
    """
    ...
    try:
      try:
        ...
        if agent in (None, identifier):
          self.agent_update_metadata(identifier, media_type, guid, id, lang, dbid, parentGUID, force)
        ...
        tasks = []
        for agent_identifier in combiner.contributing_agents(primary_metadata_cls, identifier):
          ...
          def check_agent():
            ...
            if agent in (None, agent_identifier) and agent_identifier in Core.messaging.plugin_list():
              if self.agent_handles_type(agent_identifier, media_type):
                return True
            return False
          ...
          if check_agent():
            ...
            # Otherwise, attempt to search for a match
              ...
              results = self.agent_search(agent_identifier, media_type, lang, False, kwargs)
              if MediaContainer.is_instance(results):
                if len(results) > 0:
                  item = results[0]
        ...
        metadata_class = getattr(self.access_point, media_type)
        combiner.combine(metadata_class, identifier, full_guid, self.agent_info)
  ...
  def list_contributors(self, mediaType, primaryAgent):
    """
    ...
    @lock(self.agent_info)
    def info_lock():
      if primaryAgent in self.agent_info and self.agent_handles_type(primaryAgent, media_type, primary_only=True):
        for identifier in self.agent_info:
  ...
  def media_get(self, mediaType, guid, url):
    ...
    if data != None:
      ...
      Log.Debug("Recombining the metadata bundle for %s (%s)", guid, media_type)
      combiner = self.get_combiner(lang)
      combiner.combine(getattr(self.access_point, media_type), identifier, guid)
  ...
  def agent_search(self, identifier, media_type, lang, manual, kwargs):
  ...
  def agent_update_metadata(self, identifier, media_type, guid, id, lang, dbid=None, parentGUID=None, force=False):
    return Core.messaging.call_external_function(
      identifier,
